...
LABEL_FILE_PATH = os.path.join(os.path.dirname(os.path.realpath(__file__)), "kitti_labels.txt")
...
class PreprocessYOLO(object):
    ...
    def _load_and_resize(self, input_image_path):
        ...
        image_raw = make_square(Image.open(input_image_path).convert('RGB'), min_size=new_resolution[0])
...
class PostprocessYOLO(object):
    ...
    def process(self, outputs, out_wh, resolution_raw):
        ...
        for output, owh in zip(outputs, out_wh):
            outputs_reshaped.append(self._reshape_output(output, owh))

        boxes, categories, confidences = self._process_yolo_output(outputs_reshaped, resolution_raw)

        return boxes, categories, confidences

    def _reshape_output(self, output, owh):
        ...
        owh -- output resolution [w, h]
        """
        #output = np.transpose(output, [0, 2, 3, 1])
        #_, height, width, _ = output.shape
        height, width = owh[0], owh[1]
    ...
    def _process_yolo_output(self, outputs_reshaped, resolution_raw):
        ...
        for output, mask in zip(outputs_reshaped, self.masks):
            box = output[:,:,:,:4]
            category = np.expand_dims(output[:,:,:,4], axis=-1)
            confidence = output[:,:,:,5:]
    ...
    def _filter_boxes(self, boxes, box_confidences, box_class_probs):
        ...
        pos = np.where(box_confidences[:,:,:,0] >= self.object_threshold)
