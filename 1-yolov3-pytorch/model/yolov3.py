...
class DarkNet53(nn.Module):
    ...
    class YoloLayer(nn.Module):
        def __init__(self, yolo_idx, layer_info, stride, in_width, in_height):
            ...
            self.box_attr = self.n_classes + 5  # 각 클래스에 속할 확률(클래스 개수만큼) + objectiveness + bbox 정보(x, y, w, h)
    ...
    ### 모델 cfg 파일에 따라 각 층을 쌓음
    def set_layer(self, layer_info):
        module_list = nn.ModuleList()
        in_channels = [self.in_channels]    # 이전 층의 출력 채널이 다음 층의 입력 채널이 됨. 이를 저장
        yolo_idx = 0
    ...
    def initialize_weights(self):
        for m in self.modules():    # track all layers
    ...
    # weights_path에 저장된 가중치들을 parse & load. weight가 binary로 되어 있어 이를 처리하기 위함
