...
class Evaluator:
    ...
    def run(self):
        ...
        for i, batch in enumerate(self.eval_loader):
            if batch is None:
                continue    #drop the invalid frames
            ...
            targets[...,2:6] = cxcy2minmax(targets[...,2:6])
                # cx, cy, w, h 형태인 바운딩박스를 xmin, xmax, ymin, ymax 형태로 변환
            ...
            targets[...,5] *= input_wh[1]
                # normalized 된 박스 정보를 정상화

            start_time = time.time()

            with torch.no_grad():
                output = self.model(input_img)
                best_box_list = non_max_suppression(output, conf_thres=0.1, iou_thres=0.5)  # 비최대억제를 사용해 bbox를 추론
                
            predict_all += get_batch_statistics(best_box_list, targets, iou_threshold=0.5)  # 정확도 등 점수 산출
